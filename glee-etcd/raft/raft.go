...
func newRaft(c *Config) *raft {
	...
	r.leaderTransferTimeout = r.electionTimeout * 100 //electionTimeout是1s leaderTransferTimeout是100s
...
func stepLeader(r *raft, m pb.Message) error {
	...
	case pb.MsgHeartbeatResp:
		...
		r.logger.Infof("m.RttMap string  is " + string(m.Rttmap))
		err := json.Unmarshal(m.Rttmap, &rttMap)
		if err != nil {
			r.logger.Infof("%v receive rttMap error", r.id)
		}
		if len(rttMap) != 0 {
			for id, value := range rttMap {
				...
				r.logger.Infof("test Rtt from %v to %v is %d us.", id, m.From, value)
			}
		}
...
func (r *raft) handleHeartbeat(m pb.Message) {
	...
	for id, value := range r.rttMap {
		if value.timeStamp == 0 {
			r.logger.Infof("test send Rttinfo from %v to %v is %d us. value is 0", id, m.From, value.timeStamp)
			continue
		}
		rttMapInfo[id] = value.timeStamp
		r.logger.Infof("test send Rttinfo from %v to %v is %d us.", id, m.From, value.timeStamp)
	}
	rttmMapBytes, err := json.Marshal(rttMapInfo)
	//r.logger.Infof("send rttMapBytes" + string(rttmMapBytes))
	if err != nil {
		r.logger.Infof("%x send rttMap error", r.id)
	}
...
func (r *raft) calcRtt(id uint64) {
	...
	if len(leaderRTTInfo) < n/2 {
		...
		r.electionPenaltyTime = int(leader_median) / 100 //electionTimeout为10 代表1s    一个tick 100ms
		r.randomizedElectionTimeout = r.electionTimeout + r.electionPenaltyTime
		r.logger.Infof("delay-random-timeout is %d", r.randomizedElectionTimeout)
	}
...
func (r *raft) Determine_network_deterioration() bool {
	...
	r.logger.Infof("test R_1 is %f.", R_1)
	if R_1 < 0.0003 {
		return false
	} else {
		return true
...
func (r *raft) CalBestNode(BetterN []uint64) uint64 {
	...
	leaderRTTInfo := make([]int64, 0)
	n := len(r.prs.VoterNodes())
	r.logger.Infof("test node num n is %d.", n)
	BestNode := r.id
	...
	if len(leaderRTTInfo) < n/2 {
		...
		for _, id := range BetterN {
			...
			if len(values) < n/2 {
				r.logger.Infof("node %v hasn't enough rtt info", id)
				continue
			}
			// 计算中位数
...
func calculateMedian(values []int64, n int) float64 {
	...
	return float64(values[0]) //values中没有节点本身时延
}
